...
class MultipleObjectTrackingEval(Eval):
    ...
    def update_op(self, net_output, img_info):
        for i in range(len(img_info['video_name'])):
            ...
            if video_name not in self._video_trackers:
                self._video_trackers[video_name] = JDETracker(MAX_DETECTIONS_PER_IMAGE, TRACK_BUFFER_SIZE, MEAN, STD,
    ...
    def evaluate(self):
        summary = Evaluator.get_summary([self._evaluators[name].acc for name in self._evaluators],
                                        [name for name in self._evaluators])
