...
class LaneDetectionEval(Eval):
    ...
    def __init__(self, **kwargs):
        ...
        self._metrics_vals = [0.0, 0.0, 0.0]
    ...
    def _reduce_lanes_to_labels(self, pred_lanes, y_samples, img_h):
        ...
        reduced_pred_lanes = []
        for _idx, lane in enumerate(pred_lanes):
            reduced_lane = []
    ...
    def _bench(self, pred_lanes, gt, gt_org_lanes, y_samples, img_w, pred_confs):
        ...
        for gt_org_lane, thresh in zip(gt_org_lanes, thresholds, strict=True):
            ...
            max_acc = np.max(accs) if len(accs) > 0 else 0.0  # the prediction with the highest acc is chosen.
        ...
        return [
            s / max(min(4.0, len(gt_org_lanes)), 1.0),
            fp / len(pred_lanes) if len(pred_lanes) > 0 else 0.0,
            fn / max(min(len(gt_org_lanes), 4.0), 1.0),
        ]
    ...
    def _bench_one_submit(self, pred, gt):
        ...
        gt_org_lanes = gt["gt_lanes"]  # only x values, including negative
    ...
    def _bench_one_submit_tusimple(self, pred_lanes, gt):
        ...
        if len(gt_org_lanes) > 4:
            s -= min(line_accs)  # together with the strangeness above: in case of 5 lanes,
        # they allow low-acc for 1 lane and remove it from calc.
        _acc = s / max(min(4.0, len(gt_org_lanes)), 1.0)
        _fp = fp / len(pred_lanes) if len(pred_lanes) > 0 else 0.0
        _fn = fn / max(min(len(gt_org_lanes), 4.0), 1.0)
        self._images_seen += 1
        self._accuracy += _acc
        self._fp += _fp
        self._fn += _fn
    ...
    def _get_lanes_tusimple(self, seg_out, h_samples, samp_factor):
        ...
        lanes = []
        for idx, _t_id in enumerate(lane_ids):
