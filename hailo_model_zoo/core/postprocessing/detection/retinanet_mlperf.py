    ...
    for i, BoxTensor in enumerate(sorted_output_branches):
        ...
        if i % 2 == 0:
            reshaped_tensor = tf.reshape(
                BoxTensor, shape=[-1, branch_h * branch_w * tf.cast(branch_features / 4, tf.int32), 4]
            )
        ...
        # Even locations are the class preidctors
            ...
            reshaped_tensor = tf.reshape(
                BoxTensor,
                shape=[-1, branch_h * branch_w * tf.cast(branch_features / (num_classes), tf.int32), num_classes],
            )
...
class AnchorGen(object):
    ...
    def calc_feature_maps_size(self):
        ...
        for ratio in [1 / 8, 1 / 16, 1 / 32, 1 / 64, 1 / 128]:
            features_maps_size.append(
                [math.ceil(self.orig_image_size[0] * ratio), math.ceil(self.orig_image_size[1] * ratio)]
            )
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            anchors.append(
                np.reshape(np.reshape(shifts, (-1, 1, 4)) + np.reshape(base_anchors, (1, -1, 4)), (-1, 4)).astype(
                    np.float32
                )
            )
    ...
    def get_anchors(self, image_size, grid_sizes):
        strides = [
            [np.asarray(image_size[0] // g[0], dtype=np.float32), np.asarray(image_size[1] // g[1], dtype=np.float32)]
            for g in grid_sizes
        ]
...
class retinanet_postproc(object):
    ...
    def _decode_boxes(self, rel_codes, anchors):
        ...
        dw = tf.clip_by_value(dw, clip_value_min=tf.math.reduce_min(dw), clip_value_max=tf.math.log(1000.0 / 16))
        dh = tf.clip_by_value(dh, clip_value_min=tf.math.reduce_min(dh), clip_value_max=tf.math.log(1000.0 / 16))
        w = tf.exp(dw) * wa
        h = tf.exp(dh) * ha
    ...
    def clip_boxes_to_image(self, boxes):
        return tf.clip_by_value(
            ...
            clip_value_max=[self._image_dims[0], self._image_dims[1], self._image_dims[0], self._image_dims[1]],
        )
        ...
        with tf.name_scope("Postprocessor"):
            ...
            for class_per_level, box_per_level, anchors_per_level in zip(
                ...
                top_k_scores, top_k_box, top_k_anchors = tf.numpy_function(
                    ...
                    [class_per_level, box_per_level, anchors_per_level],
