...
CLASSES = ['apple', 'aquarium fish', 'baby', 'bear', 'beaver', 'bed', 'bee', 'beetle', 'bicycle',
           ...
           'forest', 'fox', 'girl', 'hamster', 'house', 'kangaroo', 'keyboard', 'lamp', 'lawn mower',
           'leopard', 'lion', 'lizard', 'lobster', 'man', 'maple tree', 'motorcycle', 'mountain',
           'mouse', 'mushroom', 'oak tree', 'orange', 'orchid', 'otter', 'palm tree', 'pear',
...
def _generate_classe_tokens(model, classnames, templates):
    zeroshot_weights = []
...
def _create_class_tokens(model_name):
    ...
    (tfrecords_filename.parent).mkdir(parents=True, exist_ok=True)
    class_tokens = _generate_classe_tokens(model, CLASSES, TEMPLATES)
...
if __name__ == '__main__':
    ...
    parser.add_argument('type', help="TFRecord of which dataset to create", type=str,
                        choices=TF_RECORD_TYPE)
    parser.add_argument('--model', help="CLIP model to use", type=str, default='RN50',
...
python create_cifat100_tfrecord.py
-----------------------------------------------------------------
"""
