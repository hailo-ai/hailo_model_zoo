...
def point2center(xmin, ymin, xmax, ymax):
    ...
    return xmin + width / 2.0, ymin + height / 2.0, width, height


class DefaultBoxes(object):
    def __init__(
        self, img_shape, layers_shapes, anchor_scales, extra_anchor_scales, anchor_ratios, layer_steps, clip=False
    ):
    ...
    def get_layer_anchors(self, layer_shape, anchor_scale, extra_anchor_scale, anchor_ratio, layer_step, offset=0.5):
        ...
        with tf.name_scope("get_layer_anchors"):
            ...
            num_anchors_along_depth = (
                2 * len(anchor_scale) * len(anchor_ratio) + len(anchor_scale) + len(extra_anchor_scale)
            )
            ...
            return (
                ...
                tf.expand_dims(y_on_image, axis=-1),
    ...
    def get_all_anchors(self):
        ...
        for ind, layer_shape in enumerate(self._layers_shapes):
            anchors_in_layer = self.get_layer_anchors(
                layer_shape,
            ...
            anchors_xmin, anchors_ymin, anchors_xmax, anchors_ymax = center2point(
                anchors_in_layer[0], anchors_in_layer[1], anchors_in_layer[2], anchors_in_layer[3]
            )
...
def parse_by_class_fixed_bboxes(cls_pred, bboxes_pred, params):
    ...
    detection_scores, idxes = tf.nn.top_k(
        detection_scores, k=tf.minimum(params["keep_max_boxes"], num_bboxes), sorted=True
    )
    ...
    detection_bboxes = tf.cond(
        cur_num < keep_max_boxes,
        lambda: tf.concat(
            [detection_bboxes, tf.zeros(shape=(params["keep_max_boxes"] - cur_num, 4), dtype=tf.float32)], axis=0
        ),
        lambda: detection_bboxes,
    )
    detection_scores = tf.cond(
        cur_num < keep_max_boxes,
        lambda: tf.concat(
            [detection_scores, tf.zeros(shape=(params["keep_max_boxes"] - cur_num,), dtype=tf.float32)], axis=0
        ),
        lambda: detection_scores,
    )
    detection_classes = tf.cond(
        cur_num < keep_max_boxes,
        lambda: tf.concat(
            [detection_classes, tf.zeros(shape=(params["keep_max_boxes"] - cur_num,), dtype=tf.int32)], axis=0
        ),
        lambda: detection_classes,
    )
...
def parse_by_class(cls_pred, bboxes_pred, num_classes, select_threshold, min_size, keep_topk, nms_topk, nms_threshold):
    with tf.name_scope("select_bboxes"):
        ...
        for class_ind in range(1, num_classes):
            ...
            xmin, ymin, xmax, ymax, selected_scores[class_ind] = sort_bboxes(
                selected_scores[class_ind], xmin, ymin, xmax, ymax, keep_topk, "sort_bboxes_{}".format(class_ind)
            )
...
class SSDMLPerfPostProc:
    ...
    def postprocessing(self, endnodes, **kwargs):
        ...
        neg_threshold = 0.5
        out_shape = (1200, 1200)
        defaultboxes_creator = DefaultBoxes(
            out_shape,
        ...
        with tf.device("/cpu:0"):
            bboxes_pred = tf.map_fn(lambda _preds: decode_fn(_preds), location_pred, dtype=tf.float32, back_prop=False)
            # bboxes_pred = tf.concat(bboxes_pred, axis=1)

            def parse_bboxes_fn(x):
                return parse_by_class_fixed_bboxes(x[0], x[1], params)

            pred_results = tf.map_fn(
                parse_bboxes_fn,
                (cls_pred, bboxes_pred),
